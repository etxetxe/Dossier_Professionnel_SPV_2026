...
Les règles pour construire et maintenir le système de publication. Ceci est la source canon. Chaque espace de travail suit ces patterns.

---

## 5 couches de routine architecture

l'Agents lit sous la couche. Il s'arrête autant que nécessaire.

```
Couche 0: CLAUDE.md    -> "Qui suis-je ?"      (toujours chargé, ~800 tokens)
Couche 1: CONTEXT.md   -> "Où vais-je ?"  (lecture au démarrage, ~300 tokens)
Couche 2: Stage CONTEXT.md    -> "Qu'est-ce que je fais ?"   (lire tâche par tâche, ~200-500 tokens)
Couche 3: Référence matériel  -> "Quelles règles j'applique ?"  (varie selon le chargement de la sélection)
Couche 4: Artefacts utilisés   -> "Avec quoi je dois bosser ?"  (varie selon le chargement de la sélection)
```

**Layer 0 -- CLAUDE.md** est automatiquement chargé par Claude Code à chaque conversation. Il contient l'arborescence, les conventions de nommage, et une table adressant vers les fichiers des niveaux d'espace de travail. Un par espace de travail.

**Layer 1 -- Top-niveau CONTEXT.md** est la première chose qu'un agent lit quand il entre dans l'espace de travail. Il contient une routine de tâche adressant les types de tâche par dossier. Un par espace de travail.

**Layer 2 -- Etage CONTEXT.md files** vit dans chaque dossier; Ils contiennent les scopes définis, les tables de chargement, et un processus étape-par-étape. Un par étage. la Couche 2 est le point de contrôle du système -- Sa Table d'entrée détermine exactement quel fichier des Couches 3 et 4 l'agent charge.
